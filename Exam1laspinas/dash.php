...
if (@$_GET['q'] == 0) {
    ...
    $result = mysqli_query($con, "SELECT * FROM quizlaspinas ORDER BY date DESC") or die('Error');
...
if (@$_GET['q'] == 1) {
    ...
    $result = mysqli_query($con, "SELECT * FROM userslaspinas") or die('Error');
...
if (@$_GET['fid']) {
    echo '<br />';
    $id = @$_GET['fid'];
    $result = mysqli_query($con, "SELECT * FROM feedbacklaspinas WHERE id='$id' ") or die('Error');
    while ($row = mysqli_fetch_array($result)) {
        $name     = $row['name'];
        $subject  = $row['subject'];
        $date     = $row['date'];
...
if (@$_GET['q'] == 6) {
   $q=mysqli_query($con,"SELECT * FROM historylaspinas" )or die('Error197');
echo  '<div class="panel title" style="background-color: lightyellow;">
<table class="table table-striped title1" >
...
$qa=$row['level'];
$q23=mysqli_query($con,"SELECT title FROM quizlaspinas WHERE  eid='$eid' " )or die('Error208');
