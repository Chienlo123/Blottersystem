...
if(isset($_POST['submit']))
{
    if(!empty($_POST['name'] && !empty($_POST['email']&& !empty($_POST['birthday']&& !empty($_POST['phone']&& !empty($_POST['address']&& !empty($_POST['license_type']&& !empty($_POST['lesson_type']&& !empty($_POST['time']&& !empty($_POST['date']))))))))))
    {
        ...
        $query = "insert into enrollmentlaspinas(name,email,birthday,phone,address,license_type,lesson_type,time,date) values('$name','$email','$birthday','$phone','$address','$license_type','$lesson_type','$time','$date')";
