...
<?php 
 ...
 if(isset($_POST['login'])){
   ...
   $query = "SELECT * FROM userslaspinas WHERE username = '$username' AND password='$password'";
    $run = mysqli_query($connection,$query);
    $row = mysqli_num_rows($run);
    if($row < 1)
    {
                   echo "<script>alert('Username Or Password Wrongs')</script>";
    
                  echo "<script> document.location.href='../indexlaspinas.php';</script>";
    }
    else{
	  ...
	  if ($db_user_role == 'Admin') {
				...
				echo "<script>alert('You Are Successfully logged')</script>";
                  ...
                  echo "<script> document.location.href='../Learners/indexlaspinas.php';</script>";
