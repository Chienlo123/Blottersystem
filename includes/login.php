...
<?php 
 ...
 if(isset($_POST['login'])){
   ...
   $query = "SELECT * FROM usersmarikina WHERE username = '$username' AND password='$password'";
    ...
    if($row < 1)
    {
	  ...
	  if ($db_user_role == 'Admin') {
			...
			else if ($db_user_role == 'Staff') {
				echo "<script>alert('You Are Successfully logged in as Staff')</script>";
