<?php 
	...
	if (isset($_POST['insert-Learners'])) {
		...
		if ($category=="" || $Learners_Name=="" || $District=="" || $Address=="" || $Motorcycle=="" || $Light=="" || $Heavy=="" || $detail=="") {
			...
			$query = "INSERT INTO postspila(user_id, post_category_id, post_title, post_author, post_image, post_content, Price_m, Price_l, Price_h, Address, District, City) VALUES('{$curr_user_id}', '{$category}', '{$Learners_Name}', '{$user_name}', '{$image}', '{$detail}', '{$Motorcycle}','{$Light}','{$Heavy}','{$Address}','{$District}','{$City}')";
